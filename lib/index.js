...
class Processor {
  ...
  async performOperation(binary1, binary2, operation, logger) {
    ...
    print(`  Input A: ${binary1}`, 'cyan');
    if (operation !== 'NOT') print(`  Input B: ${binary2}`, 'cyan');
    print(`  Processing:`, 'magenta');

    if (operation === 'NOT') {
      result = binary1.split('').map(bit => (bit === '0' ? '1' : '0')).join('');
      for (let i = 0; i < maxLen; i++) {
        print(`  Bit ${i} | A=${binary1[i]} → ${result[i]}`, 'magenta');
        await adaptiveDelay();
      }
    } else {
      for (let i = 0; i < maxLen; i++) {
        ...
        result += resBit;
        print(`  Bit ${i} | A=${bit1} B=${bit2} → ${operation}=${resBit}`, 'magenta');
        await adaptiveDelay();
      }
    }
    print(`  Result: ${result}`, 'green');
    await logger.logOperation(`Operation ${operation} executed. Result: ${result}`, 'info');
    return result;
  }
}
...
class OSSimulator {
  ...
  async getInput() {
    ...
    await this.logger.logOperation(`User inputs: ${originalBinary1} and ${originalBinary2}`, 'info');
    setProgress(60);
  ...
  async storeInRAM(result) {
    ...
    await this.logger.logOperation('Result stored in RAM', 'info');
    setProgress(90);
  }
  ...
  async displayOutput(operation, originalBinary1, originalBinary2, result) {
    ...
    print(`Original Input A: ${originalBinary1} (Full: ${originalBinary1.padStart(CONFIG.MAX_BINARY_LENGTH, '0')})`, 'yellow');
    if (operation !== 'NOT') print(`Original Input B: ${originalBinary2} (Full: ${originalBinary2.padStart(CONFIG.MAX_BINARY_LENGTH, '0')})`, 'yellow');
    const trimmedResult = result.replace(/^0+/, ''); // Remove leading zeros for display
    print(`Result: ${trimmedResult} (Full: ${result})`, 'yellow');
    print('System fully operational!', 'green');
    await this.logger.logOperation('System fully booted and operational', 'info');
    setProgress(100);
  }
